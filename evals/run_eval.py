...
Usage:
    ...
    python evals/run_eval.py --filter entry_level
    python evals/run_eval.py --filter factual --verbose

Or via main.py:
    python main.py eval
    python main.py eval --filter cert_comparison --verbose

Results are saved to evals/results/YYYY-MM-DD_HH-MM.json
"""
...
async def run_all(cases, advisor, grader, verbose=False) -> list[dict]:
    logger.info("Running %d eval cases in parallel...", len(cases))
    tasks = [run_single_case(c, advisor, grader, verbose) for c in cases]
...
async def main() -> None:
    ...
    results = await run_all(cases, advisor, grader, verbose=args.verbose)
